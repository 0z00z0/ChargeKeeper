...
internal static class ChargeControlService
{
    ...
    public static bool SetExplicitThresholds(int start, int stop, ActionCause cause)
    {
        bool ok = Primitives.ApplyExplicitThresholds(start, stop, cause);
        AppLog.Info((ok
            ? $"ChargeControl: thresholds set to {start}/{stop}"
            : $"ChargeControl: thresholds {start}/{stop} were not accepted") + cause.Clause);
        StateChanged?.Invoke();
        return ok;
    }
    ...
    public static bool ApplyPresetByName(string name, ActionCause cause)
    {
        ...
        {
            AppLog.Info($"ChargeControl: preset \"{preset.Name}\" not applied, {reason}" + cause.Clause);
            return false;
        }

        bool ok = Primitives.ApplyExplicitThresholds(preset.Start, preset.Stop, cause);
        AppLog.Info((ok
...
internal interface IChargeControlPrimitives
{
    ...
    void CancelOverride(ActionCause cause);
...
internal sealed class LiveChargeControlPrimitives : IChargeControlPrimitives
{
    ...
    public void CancelOverride(ActionCause cause) => TravelOverrideService.Cancel(cause);
